                ...
                if row.isnull().all() and table_start is not None:
                    ...
                    possible_column_names = [[
                        ...
                        ['account', 'account number', 'account name', 'counterparty'], ['reference',
                                                                                        'transaction reference', 'UTR','chq.','cheque','ref no.'],
                        ['category', 'type'],
                    ...
                    for names in column_names:
                        ...
                        if (pd.isna(row[names]) or pd.isnull(row[names])):
                            ...
                            if ((passed_cells_counter / tested_cells_counter) >= pass_value) and tested_cells_counter >= 4:

                                heading_pattern_flag=True

                                expected_headings_flag=True

                                # todo consider implementing a way of using the popped indices list to try to
                                #  identify whether it is actually a transactions table or some other kind of summary
                                #  table
